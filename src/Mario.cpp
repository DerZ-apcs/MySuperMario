...
Mario::Mario(Vector2 nposition, Vector2 nsize, MARIO_TYPE type) :
    ...
    isDucking(false), score(0), invincibilityTimer(0.0f),
    Mario_State(type), resourceManager(Singleton<ResourceManager>::getInstance())
{
    ...
    CollNorth.setSize({ size.x * 0.8f, 10 });
    CollSouth.setSize({ size.x * 0.6f, 5 });
    CollWest.setSize({ 8, size.y - 5 }); // Tăng kích thước CollWest
    CollEast.setSize({ 8, size.y - 5 }); // Tăng kích thước CollEast
    updateCollision();
...
void Mario::updateCollision()
{
    if (isDucking) {
        ...
        CollEast.setSize({ 8, size.y / 2 }); // Tăng kích thước CollEast
        CollEast.setPos({ position.x + size.x - CollEast.getWidth(), position.y + size.y * 3 / 4 - CollEast.getHeight() / 2 });
        CollWest.setSize({ 8, size.y / 2 }); // Tăng kích thước CollWest
        CollWest.setPos({ position.x, position.y + size.y * 3 / 4 - CollWest.getHeight() / 2 });
        CollSouth.setPos({ position.x + size.x / 2 - CollSouth.getWidth() / 2, position.y + size.y / 2 - CollSouth.getHeight() });
    }
    else {
        CollWest.setSize({ 8, size.y - 5 }); // Tăng kích thước CollWest
        CollEast.setSize({ 8, size.y - 5 }); // Tăng kích thước CollEast
...
void Mario::UpdateTexture()
{
    ...
    if (invincibilityTimer > 0) {
        drawTexture = (int)(invincibilityTimer * 10) % 2 == 0; // Blink every 0.1s
    }
