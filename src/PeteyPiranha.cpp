    ...
    actionTimer(PETEY_ACTION_INTERVAL)
{
    ...
    frameDuration = 0.2f;
    this->loadAnimations();
    this->enterState(PeteyPiranhaState::WALKING);
}
...
void PeteyPiranha::loadEntity(const json& j)
{
	Boss::loadEntity(j);
	currentState = static_cast<PeteyPiranhaState>(j["currentState"].get<int>());
...
void PeteyPiranha::saveEntity(json& j) const
{
    Boss::saveEntity(j);
	j["type"] = "PeteyPiranha";
