...
//
//const float BuzzyBeetle::BUZZY_SHELL_SPEED = 100.0f; // Speed when in shell state
//const float BuzzyBeetle::BUZZY_WALK_SPEED = 50.0f; // Speed when walking normally
//const float BuzzyBeetle::BUZZY_JUMP_SPEED = 200.0f; // Speed when jumping
//
//BuzzyBeetle::BuzzyBeetle(Vector2 pos, Texture2D texture):
//	Enemy(pos, { 32, 32 }, { 0, 0 }, LEFT, FALLING, texture, 0.2f, 1, GREEN),
//	reviveShakeTimer(0.0f), BuzzyState(NORMAL_BUZZY), isReviving(false), reviveTimer(0.0f)
//{
...
//{
//	return state == STATE_SHELL ? BUZZY_BEETLE_SHELL : BUZZ_NORMAL;
//}
